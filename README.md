...
**中文（输入法）**
- 中文课程（常用字 / 词语 / 短文合并为一个入口）与拼音 / 双拼练习，走系统输入法
- **就地输入**：输入框透明内嵌在正文里、贴着光标字符，输入法的预编辑（拼音串）直接画在正文上，眼睛不用在输入框和正文之间来回切
- 按输入法「提交进来的字」判定：组词中的预编辑文本不计分，全角/半角标点视为等价
- 拼音与双拼本质上是同一套输入法练习，合并成一个入口；练习页抬头可切换「键位表」，展开小鹤双拼键位参考
...
**练习页（通用）**
- 文本区右上角有「⟳ 换一段」（快捷键 `Tab`）：按当前来源重新抽一段，不用回菜单
- **自动接续**：非限时练习一段打完后自动记账并换下一段继续（火力接续），不会中断到结算页；只有限时测试到点才进结算页
- **下一段预览**：一段快打完时（中文/双拼剩 20 字、英文剩 60 字符）自动预抽下一段，在文本框空白处渐显（虚线分隔、半透明）；打完时那段从预览位置升起成为正文（0.7s，前 35% 恢复不透明），过渡期间继续打字照常显示
...
- CI（`.github/workflows/ci.yml`）：push/PR 跑 test job（fmt / clippy / 测试 / wasm 检查）；**推 `v*` tag 或手动触发**时才跑三平台矩阵（macOS arm64 + Intel 交叉编译、Linux、Windows），由 tauri-action 建草稿 release
- 网页版（`.github/workflows/pages.yml`）：每次推 main 自动部署到 GitHub Pages —— <https://nedrw.github.io/typing-fun/>，浏览器里数据走 localStorage

...
- **火力条**：打字蓄力、随时间衰减，衰减速率随蓄力上升（慢打停在中段，快打才能压满）；颜色随蓄力变鲜艳，满格进入爆发状态（100% 进入、70% 退出，避免闪烁），爆发特效的辉光与火花强度跟着最近 2.5 秒的即时速度提高。参数集中在 `src/heat.rs`，有单测与平衡关系的编译期断言
...
|         | `src/heat.rs`                  | 火力条：蓄力、蓄力相关衰减曲线、速度→爆发强度            |
